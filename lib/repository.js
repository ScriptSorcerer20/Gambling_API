...
async function createRepository(filename, {beforeCommit = async () => {}} = {}) {
    ...
    try {
        ...
        await transaction(async tx => {
            ...
            if (version === 0) {
                ...
                CREATE TABLE easter_eggs (username TEXT NOT NULL REFERENCES users(username), code TEXT NOT NULL, used_at INTEGER NOT NULL, PRIMARY KEY(username, code));
            `);
                if (old) {
                ...
                await tx.exec('CREATE TABLE easter_eggs (username TEXT NOT NULL REFERENCES users(username), code TEXT NOT NULL, used_at INTEGER NOT NULL, PRIMARY KEY(username, code)); PRAGMA user_version = 4');
            } else if (version === 2) {
                const broken = await tx.all("SELECT username, delta FROM ledger WHERE reason = 'Easter egg: KRISHD' AND delta < 0");
                for (const entry of broken) {
                    const letters = [...entry.username].filter(character => /[a-z]/i.test(character)).length;
                    const correction = -entry.delta * letters;
                    if (!Number.isSafeInteger(correction)) throw new Error('Invalid KRISHD repair amount');
                    await tx.run('UPDATE users SET money = money + ? WHERE username = ?', correction, entry.username);
                    await tx.run('INSERT INTO ledger(command_id,username,delta,reason) VALUES(?,?,?,?)', `repair:KRISHD:${entry.username}`, entry.username, correction, 'KRISHD multiplier repair');
                }
                await tx.exec("UPDATE easter_eggs SET code = 'FRETUX' WHERE code = 'FRETUKS'; PRAGMA user_version = 4");
            } else if (version === 3) {
                await tx.exec("UPDATE easter_eggs SET code = 'FRETUX' WHERE code = 'FRETUKS'; PRAGMA user_version = 4");
            }
        });
    ...
    return {
        ...
        async redeemEasterEgg(username, code) {
            const repeatable = code === 'LOSERNOOB';
            return transaction(async tx => {
                const user = await tx.get('SELECT money FROM users WHERE username = ?', username);
                if (!user) throw new AppError('USER_NOT_FOUND', 'Account no longer exists', 404);
                if (!repeatable && await tx.get('SELECT 1 FROM easter_eggs WHERE username = ? AND code = ?', username, code)) {
                    throw new AppError('EASTER_EGG_USED', 'This terminal command was already used.', 409);
                }
                const multiplier = code === 'FRETUX' ? 2 : code === 'KRISHD' ? [...username].filter(character => /[a-z]/i.test(character)).length : 1;
                const next = code === 'ILIKEMONEY' ? user.money + 1000 : user.money * multiplier;
                if (!Number.isSafeInteger(next) || next < 0) throw new AppError('BALANCE_LIMIT', 'This command would exceed the chip limit.', 409);
                const delta = next - user.money;
                if (!repeatable) await tx.run('INSERT INTO easter_eggs(username,code,used_at) VALUES(?,?,?)', username, code, Date.now());
                if (delta) {
                    await tx.run('UPDATE users SET money = ? WHERE username = ?', next, username);
                    await tx.run('INSERT INTO ledger(command_id,username,delta,reason) VALUES(?,?,?,?)', `easter:${code}:${username}`, username, delta, `Easter egg: ${code}`);
                }
                return {balance: next, effect: repeatable ? 'matrix' : 'credits'};
            });
        },
        async recover() {
            return transaction(async tx => {
