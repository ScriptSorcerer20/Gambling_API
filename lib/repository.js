...
async function createRepository(filename, {beforeCommit = async () => {}} = {}) {
    ...
    try {
        ...
        await transaction(async tx => {
            ...
            if (old) {
                ...
                await tx.exec('INSERT INTO users(username, password_hash, money) SELECT username, password_hash, money FROM legacy_users; DROP TABLE legacy_users;');
            }
    ...
    return {
        ...
        async register(username, passwordHash, sessionHash) {
            try { await transaction(tx => tx.run('INSERT INTO users(username,password_hash,session_hash) VALUES(?,?,?)', username, passwordHash, sessionHash)); }
